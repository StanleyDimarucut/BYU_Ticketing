...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($title === '' || $description === '') {
        ...
        $stmt = $conn->prepare('UPDATE tickets SET subject = ?, description = ?, category = ?, priority = ?, importance = ? WHERE id = ?');
        $stmt->bind_param('sssssi', $title, $description, $category, $priority, $importance, $ticket_id);
